...
@api_view(['GET','POST'])
def form_create(request,id):
    loginuser=session_user_id(request)
    print(loginuser)
    print(id)
    processcard_obj = Processcard.objects.get(pk=id)
    processcard_data = ProcesscardSerializer(processcard_obj).data
    comments_obj = Comments.objects.get(pk=id)
    comments_data = CommentsSerializer(comments_obj).data
    
    if request.method=='GET':
        if loginuser.has_perm('printerapp.add_gsm'):
            print("yes")
            return Response({'data':processcard_data,'module':'Form','comments_data':comments_data},template_name='form/create_update.html')
        else:
            print("no")
        ...
        if serializer.is_valid():
            ...
            if request.accepted_renderer.format=='html':
                return Response({"success_data": "Data added successfully"},template_name='form/create_update.html')
...
def form_list(request):
    ...
    custom_filter['assigned_to_id']=loginid
    ...
    paginator = Paginator(processcard_data, row_per_page)
    try:
    ...
    if loginuser.has_perm('printerapp.list_gsm'):
        print("yes")                
        if request.accepted_renderer.format == 'html':
            return Response({"data":processcard_data,'module':'Form',"custom_filter":custom_filter},template_name='form/list.html')
